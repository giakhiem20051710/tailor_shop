...
const CustomerOrderPage = () => {
  ...
  const [galleryStyles, setGalleryStyles] = useState({
    ao_dai: [],
    vest: [],
    dam: []
  });
  ...
  const FALLBACK_STYLES = {
    ao_dai: [
      { id: 1, name: "Áo dài cưới trắng (Mẫu)", image: "/images/styles/ao-dai-1.jpg", tags: ["cưới", "trắng"] },
      { id: 2, name: "Áo dài đỏ truyền thống (Mẫu)", image: "/images/styles/ao-dai-2.jpg", tags: ["truyền thống", "đỏ"] }
    ],
    vest: [
      { id: 4, name: "Vest công sở đen (Mẫu)", image: "/images/styles/vest-1.jpg", tags: ["công sở", "đen"] }
    ],
    dam: [
      { id: 7, name: "Đầm dạ hội đen (Mẫu)", image: "/images/styles/dam-1.jpg", tags: ["dạ hội", "đen"] }
    ]
  };

  const fetchGalleryStyles = async () => {
    if (galleryStyles.ao_dai.length > 0) return; // Already fetched

    setIsLoadingGallery(true);
    try {
      // Use imageAssetService to fetch style images
      const [aoDai, vest, dam] = await Promise.all([
        imageAssetService.filter({ category: 'Áo dài' }, { size: 6 }),
        imageAssetService.filter({ category: 'Vest' }, { size: 6 }),
        imageAssetService.filter({ category: 'Váy' }, { size: 6 })
      ]);

      const parseAssets = (res, fallbackKey) => {
        const assets = res?.content || res?.data?.content || [];
        if (assets.length === 0) return FALLBACK_STYLES[fallbackKey] || [];

        return assets.map(a => ({
          id: a.id,
          name: a.description || a.type || "Mẫu tham khảo",
          image: a.url || a.thumbnailUrl || '/images/placeholder-fabric.jpg',
          tags: a.tags || [a.category || fallbackKey]
        }));
      };

      setGalleryStyles({
        ao_dai: parseAssets(aoDai, 'ao_dai'),
        vest: parseAssets(vest, 'vest'),
        dam: parseAssets(dam, 'dam')
      });
    } catch (err) {
      console.error("Failed to fetch gallery styles", err);
      // Fallback to offline data if API fails
      setGalleryStyles(FALLBACK_STYLES);
    } finally {
              ...
              <div className="grid md:grid-cols-2 gap-4 mb-6">
                <div className="bg-black rounded-xl overflow-hidden aspect-video">
                  <iframe
                    ...
                    src="https://www.youtube.com/embed/6du61S8O3ZU"
                    title="Video hướng dẫn đo 1"
                ...
                <div className="bg-black rounded-xl overflow-hidden aspect-video">
                  <iframe
                    className="w-full h-full"
                    src="https://www.youtube.com/embed/RFfauIdaqkE"
                    title="Video hướng dẫn đo 2"
                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                    allowFullScreen
                  ></iframe>
                </div>
            ...
            <div className="bg-white rounded-2xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-6">
                ...
                <div className="flex gap-2 mb-6 flex-wrap">
                  {Object.keys(galleryStyles).map(category => (
                    <button
                      key={category}
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {isLoadingGallery ? (
                    <div className="col-span-full py-12 text-center text-gray-400">
                      <div className="w-8 h-8 border-2 border-gray-300 border-t-amber-500 rounded-full animate-spin mx-auto mb-2"></div>
                      Đang tải mẫu...
                    </div>
                  ) : (galleryStyles[formData.selectedProductType] || []).map(style => {
                    ...
                    return (
                      ...
                      >
                        <div className="aspect-[3/4] bg-gray-100 flex items-center justify-center overflow-hidden">
                          {style.image && !style.image.includes('placeholder-fabric') ? (
                            <img src={style.image} alt={style.name} className="w-full h-full object-cover hover:scale-110 transition-transform duration-500" />
                          ) : (
                ...
                <div className="mt-6 p-4 bg-gray-50 rounded-xl">
                  ...
                  <div className="mt-3">
                    ...
                    {showLinkGuide && (
                      <div className="mt-2 p-3 bg-white rounded-lg border border-gray-200 text-xs text-gray-600 animate-fade-in-down">
                        <div className="mb-2">
                          <strong className="text-gray-900 block mb-1">Pinterest:</strong>
                          <p>Mở ảnh {'>'} Nhấn nút Chia sẻ (Share) {'>'} Sao chép liên kết (Copy Link)</p>
                        </div>
                        <div>
                          <strong className="text-gray-900 block mb-1">Instagram:</strong>
                          <p>Mở ảnh {'>'} Nhấn dấu 3 chấm (...) {'>'} Liên kết (Link/Copy Link)</p>
                        </div>
                      </div>
                    )}
                  </div>
