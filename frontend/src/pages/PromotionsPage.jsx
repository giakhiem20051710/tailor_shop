...
const PromotionsPage = () => {
  ...
  useEffect(() => {
    const fetchDynamicImages = async () => {
      try {
        // Fetch images from image_asset table using imageAssetService
        ...
        if (items.length > 0) {
          // Helper to find matching images based on type/occasion fields from ImageAsset
          const findImages = (keywords, count = 1) => {
            return items.filter(i =>
              keywords.some(k =>
                (i.type && i.type.toLowerCase().includes(k)) ||
                (i.occasion && i.occasion.toLowerCase().includes(k)) ||
                (i.gender && i.gender.toLowerCase().includes(k))
              )
            ).slice(0, count);
          };

          const getRandom = (arr) => arr[Math.floor(Math.random() * arr.length)];
          const getValidImage = (img) => img?.url || img?.thumbnailUrl;

          // 1. Update Promotions
          setPromotions(prev => prev.map(p => {
            let keywords = [];
            if (p.occasionKey === 'tet') keywords = ['tet', 'ao_dai', 'ao dai', 'wedding'];
            else if (p.occasionKey === 'wedding-season') keywords = ['wedding', 'cuoi', 'vest', 'ao_dai'];
            else if (p.occasionKey === 'back-to-work') keywords = ['work', 'office', 'vest', 'blazer', 'ao_so_mi'];
            else if (p.occasionKey === 'women-day') keywords = ['daily', 'party', 'dress', 'dam', 'female'];
            else if (p.occasionKey === 'year-end') keywords = ['party', 'da_hoi', 'dam', 'evening'];
            else if (p.occasionKey === 'birthday') keywords = ['party', 'casual', 'vest', 'daily'];
          ...
          setEventBanners(prev => prev.map(p => {
            let keywords = [];
            if (p.tag.includes('Tết')) keywords = ['tet', 'ao_dai', 'wedding'];
            else if (p.tag.includes('Wedding')) keywords = ['wedding', 'cuoi', 'ao_dai'];
            else if (p.tag.includes('Office')) keywords = ['work', 'office', 'vest', 'blazer'];
            else if (p.tag.includes('Lễ hội')) keywords = ['party', 'da_hoi', 'evening'];
