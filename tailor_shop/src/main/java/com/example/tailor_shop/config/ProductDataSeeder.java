    ...
    private void seedFabrics() {
        ...
        for (int i = 0; i < 50; i++) {
            ...
            FabricEntity fabric = FabricEntity.builder()
                ...
                .pricePerMeter(BigDecimal.valueOf(200000 + faker.number().numberBetween(0, 800000))) // 200k-1M VND/m
    ...
    private String getTemplateImageAndUpload(String category, String name, int index) {
        ...
        if (uploadToS3) {
            try {
                ...
                log.warn("⚠️ Failed to upload template image for {}: {}. Using Unsplash URL directly.", name, e.getMessage());
            }
        }
