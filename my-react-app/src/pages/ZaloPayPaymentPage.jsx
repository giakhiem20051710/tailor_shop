...
export default function ZaloPayPaymentPage() {
  ...
  useEffect(() => {
    // Tạo QR code cho ZaloPay (giả lập)
    const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=zalopay://payment?amount=${finalTotal}&orderId=${Date.now()}`;
    setQrCodeUrl(qrUrl);
  }, [finalTotal]);

  const handlePayment = () => {
    setIsProcessing(true);
    // Giả lập mở app ZaloPay
    setTimeout(() => {
      alert("Đang mở ứng dụng ZaloPay...");
      // Trong thực tế sẽ mở deep link: zalopay://payment?...
    }, 1000);
  };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="flex items-center justify-center mb-6">
                <div className="bg-[#0068FF] text-white rounded-lg px-6 py-4">
                  <div className="text-[24px] font-bold">ZaloPay</div>
                </div>
              </div>

              {/* QR Code */}
              <div className="flex justify-center mb-6">
                <div className="border-2 border-dashed border-[#0068FF] p-4 rounded-lg bg-white">
                  {qrCodeUrl ? (
                    <img 
                      src={qrCodeUrl} 
                      alt="ZaloPay QR Code" 
              ...
              <div className="space-y-4 mb-6">
                <div className="bg-[#F9FAFB] rounded-lg p-4">
                  ...
                  <ul className="text-[13px] text-[#6B7280] space-y-2">
                    <li className="flex items-start gap-2">
                      <span className="text-[#0068FF]">1.</span>
                      <span>Mở ứng dụng ZaloPay trên điện thoại</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-[#0068FF]">2.</span>
                      <span>Quét mã QR code bên trên hoặc nhấn nút "Thanh toán" để mở app</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-[#0068FF]">3.</span>
                      <span>Xác nhận thông tin thanh toán trong app ZaloPay</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-[#0068FF]">4.</span>
                ...
                <div className="border border-[#E5E7EB] rounded-lg p-4">
                  ...
                  <div className="text-[12px] text-[#6B7280] mt-2">
                    Thanh toán nhanh chóng và an toàn với ZaloPay
                  </div>
                </div>
              </div>
              ...
              <button
                ...
                className="w-full py-3 bg-[#0068FF] text-white rounded-lg font-medium hover:bg-[#0052CC] transition-colors disabled:bg-[#D1D5DB] disabled:cursor-not-allowed"
              >
                {isProcessing ? "Đang xử lý..." : "Mở ZaloPay để thanh toán"}
              </button>
            </div>
